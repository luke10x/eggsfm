...
Music and SFX volumes are independent and controlled separately.
...
Both methods are thread-safe (atomic store) and can be called from any thread without locking the audio device.

Internally, music channels and SFX voices are generated by two separate `IngameFMChip` instances.  The audio callback generates each chip independently into a 16-bit buffer, scales each by its volume as a float multiplier, then sums them into the final output stream.  This means volume is applied **after** synthesis — the timbre of every sound is identical at 10% volume as at 100%.  There is no TL register manipulation, so no harmonic shift at lower volumes.
